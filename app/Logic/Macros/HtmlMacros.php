...
HTML::macro('show_username', function () {
    $the_username = (Auth::user()->username === Auth::user()->email) ? ((is_null(Auth::user()->first_name)) ? (Auth::user()->username) : (Auth::user()->first_name)) : (((is_null(Auth::user()->username)) ? (Auth::user()->email) : (Auth::user()->username)));
